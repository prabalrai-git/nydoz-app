...
const BuyProduct = () => {
  ...
  return (
    <div className=" mt-3 p-6">
        ...
        <table className="table align-middle table-row-dashed fs-6 gy-5 dataTable no-footer border-bottom  ">
          <thead>
            <tr className="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
              <th className="w-10px pe-2 sorting_disabled">S.N</th>
              ...
              <th className="text-end min-w-70px sorting_disabled">Action</th>
            </tr>
          </thead>
          ...
          <tbody className="fw-semibold text-gray-600">
            {data && companyProductData && (
              <>
                {data?.map((product: IProductResponse, index: number) => (
                    ...
                    <td className="text-end">
                      {companyProductData?.some(
                        (companyProductItem: IProductResponse) =>
                          companyProductItem.id === product.id
                      ) ? (
                        <>
                          {oldProductListArray?.some(
                            (newProductItem: string) =>
                              newProductItem === product.id
                          ) ? (
                            <button
                              onClick={() => handleSelectOldProduct(product)}
                            ...
                            <button
                              onClick={() => handleAddOldProduct(product.id)}
                        ...
                        <>
                          {newProductListArray?.some(
                            ...
                            <button
                              onClick={() =>
                                handleNewProductRemoveFromCart(product.id)
                              }
                            ...
                            <button
                              onClick={() =>
                                handleNewProductAddToCart(product.id)
                              }
      ...
      <Modal2
        handleConfirm={() => handleRemoveOldProduct(selectedProduct?.id ?? "")}
        showChildren={true}
        show={show}
        title="Are you sure ?"
        cancelText="Cancel"
      ...
      >
        <div className="text-center">
          <p>
            ...
            <span className="fw-bold">{selectedProduct?.name}</span> from cart ?
          </p>
