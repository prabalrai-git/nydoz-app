...
const AddClientDocuments = (props: IModalProps) => {
  const {
    show,
    handleClose,
    companyId,
  ...
  const { postData, error, isLoading, updateData } =
    useMutation<IDocumentResponse>(
      `${API_ROUTE.CLIENT_DOCUMENTS}/${clientId}/documents`,
      true
    );
      ...
      <Modal.Header closeButton>
        <Modal.Title>
          <div className="text-gray-900  fs-2 fw-bold me-1">
            {selectedData?.id ? "Update Documents" : "Upload Documents"}
          </div>
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
            ...
            <div className="row">
              <div className="col-12 gap-5 gap-md-7 mb-6">
                <div className="fv-row flex-row-fluid fv-plugins-icon-container">
                  <label className="required form-label">Document Name:</label>
                  <input
                    value={title}
      ...
      <Modal.Footer>
        ...
        >
          {isLoading ? (
            ...
            <span className="mx-3">{selectedData ? "Update" : "Upload"}</span>
          )}
        </Button>
      </Modal.Footer>
    </Modal>
